...
export default function GroupCheck({
  ...
  const handleShareWinner = () => {
    ...
    const scores = results.slice(0, 5).map(r =>
      r.naam.toLowerCase() === "dennis" && siteId === "grappig" && r.realViews !== undefined ? r.realViews : r.views
    ).join(",");
    const battleUrl = `https://${site.domain}/battle?names=${encodeURIComponent(names)}&scores=${encodeURIComponent(scores)}${isNL ? "" : "&lang=en"}`;

    const text = site.share.battleText(winnerName);
  ...
  const battleLabel = isNL ? site.battle.leastLabel : "Least funny:";
    ...
    <div className={`${compact ? "mt-4" : "mt-8"} w-full max-w-sm mx-auto`}>
      <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-5">
        ...
        <p className="text-xs text-zinc-600 mb-4">
          {isNL
            ? "Voeg vrienden toe en check wie wint"
            : "Add friends and check who wins"}
        </p>
        ...
        <button
          ...
          className="mt-3 w-full rounded-lg px-4 py-2.5 text-sm font-medium text-black transition-all disabled:bg-zinc-800 disabled:text-zinc-500 disabled:cursor-not-allowed"
          style={loading || input.trim().split(/[,\n]+/).filter(n => n.trim()).length < 2 ? {} : { backgroundColor: site.accentColor }}
        >
          {loading ? "..." : "Check"}
        </button>
          ...
          <div className="mt-4 rounded-lg bg-zinc-800/50 p-4">
            {winner && winner.views > 0 ? (
              <>
                ...
                <p className="text-lg font-bold" style={{ color: site.accentColor }}>
                    ...
                    {results.slice(1, 5).map((r, i) => (
                      <div key={r.naam} className="flex justify-between text-xs text-zinc-500">
                        <span>
                          ...
                          {r.naam.toLowerCase() === "dennis" && siteId === "grappig" && (
                ...
                <button
                  ...
                  className="mt-4 w-full flex items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-medium text-white transition-all hover:opacity-90"
