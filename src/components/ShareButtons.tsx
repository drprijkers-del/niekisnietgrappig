...
function getHomeUrl(siteId: SiteId) {
  ...
  // Check if on a subdomain of this site's domain
  if (hostname.match(new RegExp(`\\.${site.domain.replace(/\./g, "\\.")}$`))) {
    return `https://${site.domain}`;
  }
  if (site.domainEn && hostname.match(new RegExp(`\\.${site.domainEn.replace(/\./g, "\\.")}$`))) {
    return `https://${site.domainEn}`;
  }
  return "/";
}
    ...
    <section className="border-t border-zinc-800 bg-zinc-950/50 py-20 px-6">
      <div className="mx-auto max-w-md text-center">
        ...
        <div className="mt-8 pt-6 border-t border-zinc-800">
          <p className="text-sm text-zinc-500 mb-3">Werkt voor elke naam — maak er eentje voor je vrienden, collega&apos;s, of wie dan ook</p>
          <button
            onClick={() => { window.location.href = getHomeUrl(siteId); }}
            className="flex w-full items-center justify-center gap-2 rounded-full px-6 py-3 font-medium text-white transition-all hover:opacity-90"
